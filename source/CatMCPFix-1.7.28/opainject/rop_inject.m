...
void injectDylibViaRop(task_t task, pid_t pid, const char* dylibPath, vm_address_t allImageInfoAddr)
{
	...
	uint64_t dlsymAddr = remoteDlSym(task, libDyldAddr, "_dlsym");

	printf("[injectDylibViaRop] dlopen: 0x%llX, dlerror: 0x%llX, dlsym: 0x%llX\n", (unsigned long long)dlopenAddr, (unsigned long long)dlerrorAddr, (unsigned long long)dlsymAddr);
	...
	if(remoteDylibPath)
	{
		...
		if (dlopenRet) {
			...
			if (dlsymAddr) {
				size_t remoteStartSymbolSize = 0;
				vm_address_t remoteStartSymbol = writeStringToTask(task, "catmcp_touch_bridge_start", &remoteStartSymbolSize);
				if (remoteStartSymbol) {
					uint64_t startAddr = 0;
					arbCall(task, pthread, &startAddr, true, dlsymAddr, 2, (uint64_t)dlopenRet, remoteStartSymbol);
					vm_deallocate(task, remoteStartSymbol, remoteStartSymbolSize);
					printf("[injectDylibViaRop] bridge start symbol: 0x%llX\n", (unsigned long long)startAddr);
					if (startAddr) {
						uint64_t startRet = 0;
						arbCall(task, pthread, &startRet, true, startAddr, 0);
						printf("[injectDylibViaRop] bridge start returned: 0x%llX\n", (unsigned long long)startRet);
					}
				}
			}
		}
